...
import lib.dqn_model as dqn_model
from lib.structures import Experience, ExperienceBuffer, Agent
...
REPLAY_START_SIZE = 100 # Count of frames we wait for before starting training to populate the replay buffer
LEARNING_RATE = 0.0001 # learning rate used in adam optimiser
SYNC_TARGET_FRAMES = 1000 # Model sync frequency

EPSILON_DECAY_LAST_FRAME = 1500
EPSILON_START = 1.0
EPSILON_FINAL = 0.01
...
if __name__ == "__main__":
    ...
    tgt_net = dqn_model.DQN(env.observation_space.shape,
                            env.action_space.n).to(device)
    writer = SummaryWriter("runs/{}".format(MODEL), comment="-" + args.env)
    ...
    while True:
        ...
        print("frames : {}, epsilon : {} ".format(frame_idx, epsilon))
        ...
        if reward is not None:
            ...
            # everytime our mean rewalossrd for the last 100 episodes 
            # reaches a maximum, report this and save the model 
            # parameters
            if best_m_reward is None or best_m_reward < m_reward:
                torch.save(net.state_dict(),  exp_dir + args.env +
